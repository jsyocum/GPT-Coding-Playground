...
def update_player_position(vd):
    ...
    if vd["player_dash_timer"] > vd["DASH_COOLDOWN"]:
        vd["player_rect"].x += vd["player_x_direction"] * vd["DASH_SPEED"]
    else:
        vd["player_rect"].x += vd["player_x_velocity"]
        vd["player_rect"].y += vd["player_y_velocity"]
...
def handle_player_input(vd):
    ...
    # handle jumping
    if vd["player_y_velocity"] >= 0:
        vd["player_jumping"] = False

    if keys[pygame.K_SPACE] and vd["player_on_ground"]:
...
while True:
    ...
    # handle timers
    handle_timers(variable_dictionary)

    # handle player input
    handle_player_input(variable_dictionary)

    # update player position
    update_player_position(variable_dictionary)

    # clear screen
    window.fill((0, 0, 0))

    # draw player
    draw_player(variable_dictionary)
